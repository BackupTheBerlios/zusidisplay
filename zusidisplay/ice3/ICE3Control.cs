	...
	public class ICE3Control : System.Windows.Forms.UserControl
	{
		...
		public void SetTüren(float valu)
		{
			...
			else if (localstate.Türen == TÜREN.SCHLIESSEN)
			{
				...
				if (ThreadTüren != null)
				{
					if (ThreadTüren.ThreadState == ThreadState.Stopped) 
					{
						try
						{
					...
					else if (ThreadTüren.ThreadState == ThreadState.Suspended)
					{
						ThreadTüren.Suspend();
						//DEBUG = "THREAD: SUSPEND";
					}
					//else MessageBox.Show("Thread Türen hat unerwarteten Zustand: "+ThreadTüren.ThreadState.ToString());
				}
				else
				{
					ThreadTüren = new Thread(new ThreadStart(threadDelegate_Türen));
